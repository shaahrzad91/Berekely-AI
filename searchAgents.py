        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if next_position in state[1]:
                    ...
                    success_node.append(((next_position, next_corners), action, cost))
                # if the position is not a corner
                else:
                    # just append normally, keeping the same list of corners
                    success_node.append(((next_position, state[1]), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    h = 0 # initiale heuristic = 0
    reference_curr_posi = state[0] # current position reference

    # recursive code converted to while loops below:

    # checking unvisited corners
    while corners != []:
        # split reference point into variables
        x_position, y_position = reference_curr_posi
        ...
        for x_corner, y_corner in corners:
            ...
            distances.append(manhattan(x_corner, y_corner, x_position, y_position))
        # add the distance to the closest corner to the heuristic
        h += min(distances)
        # set the reference to the closest corner
        reference_curr_posi = corners[distances.index(min(distances))]
        # remove the corner from the list of unvisited corners
        corners.remove(reference_curr_posi)
...
def foodHeuristic(state, problem):
    ...
    for food in foods:
        ...
        enjoy_pacman = position + food
        if enjoy_pacman in problem.heuristicInfo.keys():
            distance = problem.heuristicInfo[enjoy_pacman]
        else:
            distance = mazeDistance(position, food, problem.startingGameState)
            problem.heuristicInfo[enjoy_pacman] = distance
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        self.walls = gameState.getWalls()
        self.problem = AnyFoodSearchProblem(gameState)
        
        "*** YOUR CODE HERE ***"
        return search.bfs(self.problem)
